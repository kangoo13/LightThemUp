...
public abstract class MidiFileGenerator
{
	...
	public void start()
	{
        ...
        public byte[] writeToString()
        {
            try
            {
                OutputStream outputStreamString = new ByteArrayOutputStream();
                MidiSystem.write(sequencer.getSequence(), 0, outputStreamString);
                return ((ByteArrayOutputStream)outputStreamString).toByteArray();
            }
            catch (IOException e)
            {
                e.printStackTrace();
        ...
        public void writeFile(File file)
        {
            try
            {
                MidiSystem.write(sequencer.getSequence(), 0, file);
            }
            catch (IOException e)
            {
                e.printStackTrace();
