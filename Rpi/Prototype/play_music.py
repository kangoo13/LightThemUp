...
def playIt(splitted, ticks, any, oldSplitted):
...
def playSilence(ticks, oldSplitted):
    time.sleep(ticks / 1000.0)
    if oldSplitted[0] == "GPIOO":
        GPIOO.output(int(oldSplitted[1]), 0)
    elif oldSplitted[0] == "mcp20":
...
def playNote(ticks, splitted, oldSplitted):
    if splitted[0] == "GPIOO":
        playIt(splitted, ticks, GPIOO, oldSplitted)
    elif splitted[0] == "mcp20":
        playIt(splitted, ticks, mcp20, oldSplitted)
    elif splitted[0] == "mcp27":
        playIt(splitted, ticks, mcp27, oldSplitted)
    else:
        playSilence(ticks, oldSplitted)
...
for note in music:
    ...
    playNote(ticks, splitted, oldSplitted)
    if i == 0:
